...
import { fetchLiveQuote, type LiveQuote } from "@/lib/liveQuote.functions";
import { fetchScorecard } from "@/lib/scorecard.functions";
  ...
  head: ({ params }) => {
    ...
    return {
      meta: [
        ...
        {
          name: "description",
          content: `Latest earnings analysis for ${t}: call summary, themes, bull/bear cases, analyst targets, and quarter-over-quarter changes.`,
        },
        { property: "og:title", content: `${t} earnings intelligence — Earnings IQ` },
        {
          property: "og:description",
          content: `Executive-grade earnings dashboard for ${t}.`,
        },
      ],
    };
  },
...
function CompanyDashboard() {
  ...
  const liveQuoteFn = useServerFn(fetchLiveQuote);
  const analysisFn = useServerFn(fetchAnalysis);
  const scorecardFn = useServerFn(fetchScorecard);
  ...
  const {
    ...
    queryFn: () => analysisFn({ data: { ticker: upper } }),
  });
  ...
  const {
    ...
    queryFn: () => liveQuoteFn({ data: { ticker: upper } }),
    ...
    <AppShell>
      <div className="px-4 md:px-8 py-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <Link to="/search" className="inline-flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
        ...
        <div>
          ...
          {a.isPreview && (
            <div className="mt-2 inline-flex items-center gap-1.5 text-[11px] px-2 py-0.5 rounded-md border border-dashed border-border text-muted-foreground">
              ...
              Preview analysis · live price, synthesized narrative
            </div>
          )}
        </div>
